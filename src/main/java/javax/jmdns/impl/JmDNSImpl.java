...
public class JmDNSImpl extends JmDNS {
  ...
  /**
   * Holds instances of JmDNS.DNSListener. Must by a synchronized collection, because it is
   * updated from concurrent threads.
   */
  private List listeners;
  /**
   * Holds instances of ServiceListener's. Keys are Strings holding a fully qualified service
   * type. Values are LinkedList's of ServiceListener's.
   */
  ...
  /**
   * This hashtable holds the services that have been registered. Keys are instances of String
   * which hold an all lower-case version of the fully qualified service name. Values are
   * instances of ServiceInfo.
   */
  Map services;

  /**
   * This hashtable holds the service types that have been registered or that have been received
   * in an incoming datagram. Keys are instances of String which hold an all lower-case version of
   * the fully qualified service type. Values hold the fully qualified service type.
   */
  Map serviceTypes;
  /**
  ...
  /**
   * Throttle count. This is used to count the overall number of probes sent by JmDNS. When the
   * last throttle increment happened .
   */
   ...
   * incoming DNSRecords to it, until the JmDNS.Responder timer picks it up. Remind: This does not
   * work well with multiple planned answers for packages that came in from different clients.
   */
  private DNSIncoming plannedAnswer;

  // State machine
  /**
   * The state of JmDNS. <p/> For proper handling of concurrency, this variable must be changed
   * only using methods advanceState(), revertState() and cancel().
   */
  private DNSState state = DNSState.PROBING_1;

  /**
   * Timer task associated to the host name. This is used to prevent from having multiple tasks
   * associated to the host name at the same time.
   */
  ...
  /**
   * Create an instance of JmDNS and bind it to a specific network interface given its
   * IP-address.
   */
  ...
  private boolean makeServiceNameUnique(ServiceInfoImpl info) {
    ...
    do {
      ...
      // Check for collision in cache
      for (DNSCache.CacheNode j = cache.find(info.getQualifiedName().toLowerCase()); j != null;
          j = j
              .next()) {
  ...
  /**
   * Add a listener for a question. The listener will receive updates of answers to the question
   * as they arrive, or from the cache if they are already available.
   */
  ...
  private static class ServiceCollector implements ServiceListener {
    ...
    /**
     * A service hase been resolved. Its details are now available in the ServiceInfo record.
     */
