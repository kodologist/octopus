...
public class ServiceInfoImpl extends ServiceInfo implements DNSListener {
  ...
  // State machine
  /**
   * The state of this service info. This is used only for services announced by JmDNS. <p/> For
   * proper handling of concurrency, this variable must be changed only using methods
   * advanceState(), revertState() and cancel().
   */
  private DNSState state = DNSState.PROBING_1;

  /**
   * Task associated to this service info. Possible tasks are JmDNS.Prober, JmDNS.Announcer,
   * JmDNS.Responder, JmDNS.Canceler.
   */
   ...
   */
  public ServiceInfoImpl(String type, String name, int port, int weight, int priority,
      String text) {
  ...
  public String getTextString() {
    if ((getText() == null) || (getText().length == 0) || ((getText().length == 1) && (getText()[0]
        == 0))) {
      return null;
    }
      ...
      switch (rec.type) {
        ...
        case DNSConstants.TYPE_SRV:
          if (rec.name.equals(getQualifiedName())) {
            ...
            updateRecord(jmdns, now, (DNSRecord) jmdns.getCache()
                .get(server, DNSConstants.TYPE_A, DNSConstants.CLASS_IN));
          }
          break;
