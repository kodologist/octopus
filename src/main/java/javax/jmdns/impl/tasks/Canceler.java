...
public class Canceler extends TimerTask {
  ...
  int count = 0;
  /**
   * The services that need cancelling. Note: We have to use a local variable here, because the
   * services that are canceled, are removed immediately from variable JmDNS.services.
   */
  private ServiceInfoImpl[] infos;
  /**
   * We call notifyAll() on the lock object, when we have canceled the service infos. This is used
   * by method JmDNS.unregisterService() and JmDNS.unregisterAllServices, to ensure that the JmDNS
   * socket stays open until the Canceler has canceled all services. <p/> Note: We need this lock,
   * because ServiceInfos do the transition from state ANNOUNCED to state CANCELED before we get
   * here. We could get rid of this lock, if we added a state named CANCELLING to DNSState.
   */
  private Object lock;
  int ttl = 0;
