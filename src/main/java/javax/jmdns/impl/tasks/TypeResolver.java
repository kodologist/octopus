...
/**
 * Helper class to resolve service types. <p/> The TypeResolver queries three times consecutively
 * for service types, and then removes itself from the timer. <p/> The TypeResolver will run only if
 * JmDNS is in state ANNOUNCED.
 */
    ...
    try {
      if (this.jmDNSImpl.getState() == DNSState.ANNOUNCED) {
        if (count++ < 3) {
          ...
          for (Iterator iterator = this.jmDNSImpl.getServiceTypes().values().iterator();
              iterator.hasNext(); ) {
            out.addAnswer(
                new DNSRecord.Pointer("_services._mdns._udp.local.", DNSConstants.TYPE_PTR,
