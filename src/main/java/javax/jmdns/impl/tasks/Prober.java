 ...
 * sent for it. When the prober has run three times, it launches an Announcer. <p/> If a conflict
 * during probes occurs, the affected service infos (and affected host name) are taken away from the
...
public class Prober extends TimerTask {
  ...
  public void start(Timer timer) {
    ...
    if (now - this.jmDNSImpl.getLastThrottleIncrement()
    ...
    synchronized (this.jmDNSImpl.getIoLock()) {
      ...
      try {
        ...
        if (this.jmDNSImpl.getState() == taskState && this.jmDNSImpl.getTask() == this) {
          ...
          out.addQuestion(
              new DNSQuestion(this.jmDNSImpl.getLocalHost().getName(), DNSConstants.TYPE_ANY,
        ...
        for (Iterator i = list.iterator(); i.hasNext(); ) {
          ...
          synchronized (info) {
            if (info.getState() == taskState && info.getTask() == this) {
              info.advanceState();
              logger.fine(
                  "run() JmDNS probing " + info.getQualifiedName() + " state " + info.getState());
              if (out == null) {
                out = new DNSOutgoing(DNSConstants.FLAGS_QR_QUERY);
