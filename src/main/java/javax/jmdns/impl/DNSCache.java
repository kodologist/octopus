...
/**
 * A table of DNS entries. This is a hash table which can handle multiple entries with the same
 * name. <p/> Storing multiple entries with the same name is implemented using a linked list of
 * <code>CacheNode</code>'s. <p/> The current implementation of the API of DNSCache does expose the
 * cache nodes to clients. Clients must explicitly deal with the nodes when iterating over entries
 * in the cache. Here's how to iterate over all entries in the cache:
 *
 * <pre>
...
public class DNSCache {
  ...
  /**
   * The hashtable used internally to store the entries of the cache. Keys are instances of
   * String. The String contains an unqualified service name. Values are linked lists of CacheNode
   * instances.
   */
  private final HashMap hashtable;

  /**
   * Cache nodes are used to implement storage of multiple DNSEntry's of the same name in the
   * cache.
   */
  ...
  /**
   * Iterates over all cache nodes. The iterator returns instances of DNSCache.CacheNode. Each
   * instance returned is the first node of a linked list. To retrieve all entries, one must
   * iterate over this linked list. See code snippets in the header of the class.
   */
  ...
  /**
   * Iterate only over items with matching name. Returns an instance of DNSCache.CacheNode or
   * null. If an instance is returned, it is the first node of a linked list. To retrieve all
   * entries, one must iterate over this linked list.
   */
